...
sub write_tics{ 
    ...
    my(@tics) = (0.001, 0.004, .01, 0.02, 0.05, 0.1, 0.2, 0.5,
...
sub writeMultiDetGraph{
    ...
    if (scalar(@{ $dets }) == 1){
	return($dets->[0]->writeGNUGraph($fileRoot));
    }
...
sub writeGNUGraph{
    ...
    $self->serialize("$fileRoot.srl");

    my $typeStr = ($self->{STYLE} eq "pooled" ? "Pooled $self->{TRIALS}->{BlockID} $self->{TRIALS}->{DecisionID}" : "$self->{TRIALS}->{BlockID} Weighted");
...
#    print "Writing DET to GNUPLOT file $fileRoot.*\n";
    ...
    &write_gnuplot_DET_header(*PLT, $title, 0.001, 80, 1, 90);
    ...
    print THRESHPLT "  '$fileRoot.dat.2' using 1:2 with points title 'MaxValue ".sprintf("%.3f @ %.3f",$val,$scr)."'";
...
sub buildPNG{
    ...
    system("cat $fileRoot.plt | perl -pe \'\$_ = \"set terminal png medium size 640,480\n\" if (\$_ =~ /set terminal/)\' | gnuplot > $fileRoot.png");
}
